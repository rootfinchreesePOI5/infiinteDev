    ...
    <div className='flex flex-col gap-8'>
      ...
      <div className='flex items-center gap-10'>
      {
        filterBytype.map((item , index) =>{
          return <button style={type === item ?{border:'1px solid'}:{}} className='border-2 w-[20%] lg:w-[10%] rounded-lg h-10 hover:bg-foreground text-xs hover:text-background transition-all duration-700' onClick={() =>setType(item)} key={index}>{item}</button>
        })
      }
      </div>
